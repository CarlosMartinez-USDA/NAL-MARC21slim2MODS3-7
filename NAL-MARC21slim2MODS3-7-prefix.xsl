...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
	...
	Revision 1.25 - Mapped 040e to <descriptiveStandard> and marc:leader/18 to <descriptive standard>aacr2  2008/09/18 tmee
    ...
    <xsl:template match="/">
        ...
        <xsl:param name="workingDirectory"
            select="substring-before(saxon:system-id(), tokenize(base-uri(), '/')[last()])"/>
    ...
    <xsl:template name="marcRecord">
        ...
        <!-- 1.171 languageTerm adds marc:controlfield-008-35-37 -->
        <!-- language from 008-->
        ...
        <!-- 1.180 -->
        <xsl:if test="marc:datafield[@tag='655']/marc:subfield[@code='a']!='article' and not(matches(marc:datafield[@tag='773']/marc:subfield[@code='x'][current()],'\d{4}\-\d{3}.'))">
        <xsl:choose>
            <xsl:when test="
                ...
                <!-- Cover any physical -->
                <xsl:if test="marc:datafield[@tag='655']/marc:subfield[@code='a']!='article' or not(matches(marc:datafield[@tag='773']/marc:subfield[@code='x'],'\d{4}\-\d{3}.'))">
                    <xsl:if test="
        ...
        <!-- 1.120 - @777 @787 , 1.121 -->
        <xsl:for-each select="
                marc:datafield[@tag = '760'] | marc:datafield[@tag = '775'] |
                marc:datafield[@tag = '762'] | marc:datafield[@tag = '776'] |
                marc:datafield[@tag = '765'] | marc:datafield[@tag = '777'] |
                marc:datafield[@tag = '767'] | marc:datafield[@tag = '780'] |
                marc:datafield[@tag = '770'] | marc:datafield[@tag = '785'] |
                marc:datafield[@tag = '772'] | marc:datafield[@tag = '786'] |
                marc:datafield[@tag = '773'] | marc:datafield[@tag = '787'] |
                marc:datafield[@tag = '774'] | marc:datafield[@tag = '914'] |
                ...
                marc:datafield[@tag = '880'][starts-with(marc:subfield[@code = '6'], '780')] | marc:datafield[@tag = '880'][starts-with(marc:subfield[@code = '6'], '785')] |
                marc:datafield[@tag = '880'][starts-with(marc:subfield[@code = '6'], '786')] | marc:datafield[@tag = '880'][starts-with(marc:subfield[@code = '6'], '914')]">
            <xsl:variable name="s6"
                select="substring(normalize-space(marc:subfield[@code = '6']), 5, 2)"/>
            ...
            <xsl:if test="
                    ...
                    @tag = '776' or @tag = '777' or @tag = '787' or @tag = '780' or @tag = '785' or @tag = '786' or @tag = '914'][substring(marc:subfield[@code = '6'], 5, 2) = $s6]))">
                <relatedItem>
                                        ...
                                        <xsl:matching-substring>
                                            ...
                                            <xsl:if test="matches(regex-group(4), 'no. ')">
                                                <detail type="issue">
                                                  <number>
                                                  <xsl:value-of
                                                  select="replace(substring-after(regex-group(4),'no.'), '(\s)?(\d+|pt.\s?\d+)(.*)', '$2')"
                                                  />
                                                  </number>
                                                  <caption>no.</caption>
                                                </detail>
                                            ...
                                            <xsl:choose>
                                                <!-- extent (pages) -->
                                                    ...
                                                    <xsl:when test="matches(substring-after(regex-group(4),'p.'), '\d+\-\d+')">
                                                        <xsl:variable name="startPage" select="number(translate(replace(substring-after(regex-group(4),'p.'),'(\s)?(\d+)(\-\d+)', '$2'), $alpha, ''))"/>
                                                        <xsl:variable name="endPage" select="number(translate(replace(substring-after(regex-group(4),'p.'), '(\s)?(\d+\-)(\d+)','$3'), $alpha, ''))"/>                                                  
                                                        <extent unit="pages">                                                           
                                                                <start>
                                                                ...
                                                                <end>
                                                                    ...
                                                                    <xsl:choose>                                                         
                                                                        <xsl:when test="not(f:calculateTotalPgs($startPage, $endPage) castable as xs:double)"/>
                                                                        <xsl:otherwise>
                                                                            <xsl:if test="$endPage >= $startPage">
                                                                                <total>
                                                                                  <xsl:value-of select="translate(f:calculateTotalPgs($startPage, $endPage), $alpha, '')"/>
                                                                                </total>
                                                                            </xsl:if>
                                                                        </xsl:otherwise>
                                                                    </xsl:choose>
                                               ...
                                               <xsl:otherwise>
                                                  <!-- extent (page total) -->
                                                    <xsl:if test="ends-with(regex-group(4), '-')">
                                                        <xsl:variable name="start" select="translate(substring-before(substring-after(regex-group(4), 'p.'), '-'), $alpha, '')"/>
                                                  <xsl:choose>
                                                      <xsl:when test="matches($start, '.+?[A-z]+.*')"/>
                                                      <xsl:otherwise>
                                                    <extent unit="pages">
                                                      <start>
                                                          <xsl:value-of select="translate($start, $alpha, '')"/>                                          
                                                      </start>
                                                    </extent>
                                                      </xsl:otherwise>
                                                  </xsl:choose>                                                                                                            
                                            ...
                                            <xsl:if test="tokenize(regex-group(1), '\W+')!=''">
                                                <text type="year">
                                                  <xsl:number value="tokenize(regex-group(1), '\W+')" format="0000"/>
                                                </text>
                                            </xsl:if>
            ...
            <xsl:if
                ...
                <relatedItem>
                    <xsl:for-each
                        ...
                        <!-- 1.120 - @856@ind2=2$q -->
                        <xsl:if test="marc:subfield[@code = 'q'] and marc:datafield[@tag='655']/marc:subfield[@code='a']!='article' or not(matches(marc:datafield[@tag='773']/marc:subfield[@code='x'],'\d{4}\-\d{3}.'))">
                            <physicalDescription>
                                <xsl:call-template name="xxx880"/>
                                <internetMediaType>
        ...
        <xsl:for-each select="marc:datafield[@tag = '856'][marc:subfield[@code = 'u']]">
            <xsl:if
                test="starts-with(marc:subfield[@code = 'u'][1], 'urn:hdl') or starts-with(marc:subfield[@code = 'u'][1], 'hdl') or starts-with(marc:subfield[@code = 'u'][1], 'http://hdl.loc.gov')">
                <identifier>
                    <xsl:attribute name="type">
                        <xsl:if
                            test="starts-with(marc:subfield[@code = 'u'][1], 'urn:doi') or starts-with(marc:subfield[@code = 'u'][1], 'doi')"
                            >doi</xsl:if>
                        <xsl:if
        ...
        <recordInfo>
            ...
            <recordOrigin>
                ...
                <xsl:value-of
                    select="normalize-space(concat('Converted from MARCXML to MODS version 3.7 using', ' ', $transform, ' ', '(Revision 1.180 20230216 cm3),'))"/>
                <xsl:text>&#160;</xsl:text>
                <xsl:value-of
                    select="normalize-space(concat('Transformed on: ', $dateTime))"/>
            </recordOrigin>
    ...
    <xsl:template name="personal_name">
        ...
        <xsl:for-each-group group-by="$name" select="marc:subfield[@code = '1']">
            <nameIdentifier>
                <for-each select="marc:subfield[@code='1']">
    ...
    <xsl:template name="relatedForm">
        <xsl:if test="marc:subfield[@code='h'] and marc:datafield[@tag='655']/marc:subfield[@code='a']!='article' or not(matches(marc:datafield[@tag='773']/marc:subfield[@code='x'],'\d{4}\-\d{3}.'))">
    ...
    <xsl:template name="relatedExtent">
        <xsl:for-each select="marc:subfield[@code = 'h'] and marc:datafield[@tag='655']/marc:subfield[@code='a']!='article' or not(matches(marc:datafield[@tag='773']/marc:subfield[@code='x'],'\d{4}\-\d{3}.'))">
            <physicalDescription>
                <!-- 1.121 -->
                <xsl:call-template name="xxs880"/>
                <extent>
   ...
   <!-- <xd:doc id="relatedIdentifierISSN-x" scope="component">
    ...
    <xsl:template match="marc:subfield[@code = 'x']" mode="relatedItem-x">
        <identifier type="issn">
            <!-\- 1.121 -\->
            <xsl:call-template name="xxs880"/>
            <xsl:apply-templates/>
        ...
        <!-\- 1.168 - @914$a-\->
        <xsl:if
            test="count(marc:subfield[@code = 'w']) = 0 and ../marc:datafield[@tag = '914']/marc:subfield[@code = 'a']">
            <identifier type="local">
...
&lt;/xsl:template&gt;-->
    ...
    <xd:doc>
        ...
        <xd:param name="controlField008"/>
        <xd:param name="typeOf008"/>
        <xd:variable name="marc:datafield"/>
    </xd:doc>
    <xsl:template name="originInfo">
        <!-- marc:leader and control field parameters passed from record template -->
        <xsl:param name="marcLeader6"/>
        ...
        <xsl:variable name="originInfoShared">
            ...
            <xsl:for-each-group group-starting-with=".[current()]"
                select="//marc:datafield[@tag = '773']/marc:subfield[@code = 'd']">
                <publisher>
            ...
            <!-- issuance -->
                <!--1.179 --> 
                <xsl:if test="marc:subfield[@code='h'] and marc:datafield[@tag='655']/marc:subfield[@code='a']!='article' or not(matches(marc:datafield[@tag='773']/marc:subfield[@code='x'][1],'\d{4}\-\d{3}.'))">
                <!-- 1.120 - @260$ -->
                <!-- 1.158 -->
