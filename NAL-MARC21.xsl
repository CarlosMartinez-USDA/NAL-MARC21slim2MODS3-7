...
xmlns="http://www.loc.gov/mods/v3" xmlns:f="http://functions"
	...
	Revision 1.168 - From 778 from 76X-78X template, added 914 for local identifier, "agid:" number 20230108 cm3
 ...
 2009/11/23 tmee
    ...
    <xsl:template match="/">
        <xsl:param name="workingDirectory" select="replace($originalFilename,'(.*/)(.*)(\.xml)','$1')"/>
        <xsl:result-document encoding="UTF-8" version="1.0" method="xml" media-type="text/xml"
            indent="yes" format="saxon"
            href="file:///{$workingDirectory}N-{replace($originalFilename,'(.*/)(.*)(\.xml)','$2')}_{position()}.xml">
            <xsl:choose>
    ...
    <xsl:template match="subfield[@code = 'x']" mode="relatedItem">
        ...
        <xsl:if test="//datafield[@tag ='914']/subfield[@code='a'][1]">
         <identifer type="local">
                <xsl:value-of select="//datafield[@tag ='914']/subfield[@code='a'][1]"/>
          </identifer>
        </xsl:if>
    </xsl:template>
